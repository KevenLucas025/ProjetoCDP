...
# Define a função para o botão apagar todas as pessoas fisicas
def botao_apagar_todos_fisica():
    try:
        with open('Pessoa Fisíca.csv', "w", newline='') as arquivo:
            arquivo.truncate(0)
        messagebox.showinfo("Aviso","Todas as pessoas foram apagadas com sucesso!") 
    except FileNotFoundError:
        print("Arquivo não encontrado.")
    except Exception as e:
        messagebox.showerror("ERRO", "Ocorreu um erro ao apagar todas as pessoas cadastradas.")


# Define a função para o botão apagar as pessoas juridicas
def botao_apagar_todos_juridica():
    try:
...
texto3 = ttk.Button(janela_principal, text="Apagar pessoas físicas cadastradas", style="TButton",command=botao_apagar_todos_fisica)
texto3.place(x=220, y=120)

texto4 = ttk.Button(janela_principal, text="Apagar pessoas jurídicas cadastradas", style="TButton",command=botao_apagar_todos_juridica)
